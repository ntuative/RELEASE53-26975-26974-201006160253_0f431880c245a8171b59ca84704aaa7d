package com.sulake.habbo.navigator.roomsettings
{
   ...
   public class RoomSettingsCtrl
   {
      ...
      private var var_1370:TextFieldManager;
      ...
      private var var_1369:ICheckBoxWindow;
      ...
      {
         this.var_943.clearErrors();
         this.var_1370.clearErrors();
      ...
      private function onConfirmRoomDelete(param1:WindowMouseEvent, param2:int) : void
      {
         ...
         close();
         var_439.roomSettingsRefreshNeeded();
         ...
         _navigator.toolbar.events.dispatchEvent(new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_192,HabboToolbarIconEnum.ROOMINFO));
      }
      ...
      private function onRemoveAllFlatCtrlsClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _navigator.send(new RemoveAllRightsMessageComposer(this.var_440));
      }
      
      public function onRoomSettingsSaveError(param1:int, param2:int, param3:String) : void
      {
         if(param1 != this.var_440 || var_1113 < 1)
         {
            return;
         }
         var_1113 = 0;
         if(param2 == RoomSettingsSaveErrorMessageParser.const_1425)
         {
            this.var_943.displayError("${navigator.roomsettings.roomnameismandatory}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1325)
         {
            this.var_943.displayError("${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1274)
         {
            this.var_1370.displayError("${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1423)
         {
         ...
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1298)
         {
         ...
         {
            this.var_1109.displayError("${navigator.roomsettings.passwordismandatory}");
         }
         else
         {
            this.var_943.displayError("Update failed: error " + param2);
         }
      }
      ...
      {
         Logger.log("[RoomSettingsCtrl.onDeleteButtonClick] " + var_440);
         _navigator.registerParameter("navigator.roomsettings.deleteroom.confirm.message","room_name",var_85.name);
         var _loc2_:ConfirmDialogView = new ConfirmDialogView(IFrameWindow(_navigator.getXmlWindow("ros_room_delete_confirm")),this,onConfirmRoomDelete,[var_440]);
         _loc2_.show();
      }
      ...
      private function populateForm() : void
      {
         ...
         var_943.setText(_loc1_.name);
         var_1370.setText(_loc1_.description);
         var_1109.setText("");
         var_1372.setText("");
      ...
      {
         return IWindowContainer(_navigator.windowManager.createWindow(param1,"",HabboWindowType.const_56,HabboWindowStyle.const_42,HabboWindowParam.const_75,new Rectangle(0,0,100,20)));
      }
      ...
      private function onSaveButtonClick(param1:WindowMouseEvent) : void
      {
         ...
         _loc2_.name = var_943.getText();
         _loc2_.description = var_1370.getText();
         ...
         {
            _loc8_ = var_1109.getText();
            _loc9_ = var_1372.getText();
            if(_loc8_ != _loc9_)
            {
               this.var_1109.clearErrors();
               this.var_1372.displayError("${navigator.roomsettings.invalidconfirm}");
               return;
            }
            if(_loc8_ != "")
            {
         ...
         _loc2_.allowPets = var_1373.isSelected;
         _loc2_.allowFoodConsume = var_1369.isSelected;
         _loc2_.allowWalkThrough = _allowWalkThroughCheckBox.isSelected;
         _loc2_.hideWalls = var_768.isSelected;
         _loc2_.tags = new Array();
         addTag(var_992,_loc2_.tags);
         addTag(var_991,_loc2_.tags);
         this.clearErrors();
         this.var_1113 = _loc2_.roomId;
      ...
      public function refresh(param1:IWindowContainer) : void
      {
         ...
         var_1633.visible = _inRoom;
         var_1632.visible = _inRoom;
         var_2138.visible = var_531;
         var_2137.visible = !var_531;
         if(var_531)
         {
            var_1111.visible = true;
            var_1326.visible = false;
            refreshFlatControllers();
            var_1111.height = Util.getLowestPoint(this.var_1111) + 4;
         }
         else
         {
            populateForm();
            var_1112.visible = true;
            var_1326.visible = true;
            var_1371.visible = _inRoom;
            _navigator.thumbRenderer.refreshThumbnail(IWindowContainer(var_1371.findChildByName("picframe")),_navigator.data.enteredGuestRoom.thumbnail,false);
            var_1112.height = Util.getLowestPoint(this.var_1112) + 4;
         }
         var_1631.text = !!var_531 ? "${navigator.roomsettings.tobasicsettings}" : "${navigator.roomsettings.toadvancedsettings}";
         Util.moveChildrenToColumn(_window,[var_1633.name,var_1371.name,var_1112.name,var_1111.name,var_1326.name,var_1632.name],0,0);
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_744,onDoorModePasswordSelect);
         _loc2_.addEventListener(WindowEvent.const_1051,onDoorModePasswordUnselect);
         ...
         this.var_943 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("room_name")),60);
         this.var_1370 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("description")),255);
         ...
         this.var_1109 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password")),30);
         this.var_1372 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password_confirm")),30);
         this.var_1373 = ICheckBoxWindow(_window.findChildByName("allow_pets_checkbox"));
         this.var_1369 = ICheckBoxWindow(_window.findChildByName("allow_foodconsume_checkbox"));
         this._allowWalkThroughCheckBox = ICheckBoxWindow(_window.findChildByName("allow_walk_through_checkbox"));
         this.var_768 = ICheckBoxWindow(_window.findChildByName("hide_walls_checkbox"));
         var_1633 = IWindowContainer(_window.findChildByName("header_container"));
         var_1371 = IWindowContainer(_window.findChildByName("thumbnail_container"));
         var_1112 = IWindowContainer(_window.findChildByName("basic_settings_container"));
         var_1111 = IWindowContainer(_window.findChildByName("advanced_settings_container"));
         ...
         var_1110 = IWindowContainer(_window.findChildByName("password_container"));
         var_2137 = ITextWindow(_window.findChildByName("basic_caption"));
         var_2138 = ITextWindow(_window.findChildByName("advanced_caption"));
         var_1631 = ITextWindow(_window.findChildByName("switch_view_text"));
         var_1631.procedure = onSwitchView;
         var_1110.visible = false;
         if(!_inRoom)
         {
      ...
      {
         this.var_440 = param1;
         _navigator.send(new GetRoomSettingsMessageComposer(var_440));
         if(var_1110 != null)
         {
            var_1110.visible = false;
         }
