package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      private var var_139:AvatarRenderManager;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(var_417.getValue(_loc4_) == null)
               {
                  var_417.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(var_1077.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(var_1076.indexOf(_loc6_) == -1)
               {
                  ...
                  var_257.push(_loc8_);
                  var_1076.push(_loc6_);
                  if(param3 != null && var_709.indexOf(param3) == -1)
                  {
                     var_709.push(param3);
                  }
               }
      ...
      private function onLoaderComplete(param1:Event) : void
      {
         ...
         var_1077.push(_loc2_.url);
         var _loc3_:int = var_257.indexOf(_loc2_);
         if(_loc3_ >= 0)
         {
            var_257.splice(_loc3_,1);
         }
         if(false)
         {
            var_139.resetPetData();
            notifyListeners();
         }
      }
      ...
      {
         var _loc3_:IPetData = var_417.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
            assetsReady(param1,AvatarScaleType.const_53,param2);
         }
         return _loc3_;
      }
      ...
      private function notifyListeners() : void
      {
         ...
         while(false)
         {
            _loc1_ = var_709.pop() as IPetDataListener;
            if(_loc1_ != null)
            {
               _loc1_.petDataReady();
            }
      ...
      {
         var _loc4_:PetData = var_417.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = var_417.getKeys();
         for each(key in speciesKeys)
         {
            petData = var_417.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.PollRejectComposer.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
