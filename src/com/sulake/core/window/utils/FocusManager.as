package com.sulake.core.window.utils
{
   ...
   public class FocusManager extends ChildEntityArrayReader implements IWindowFocusManagerService
   {
      ...
      public function FocusManager(param1:DisplayObject)
      {
         ...
         var_204.addEventListener(Event.ACTIVATE,onActivateEvent);
         var_204.addEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
         var_204.addEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
         var_204.addEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
         super();
      }
      ...
      public function dispose() : void
      {
         if(!_disposed)
         {
            var_204.removeEventListener(Event.ACTIVATE,onActivateEvent);
            var_204.removeEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
            var_204.removeEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
            var_204.removeEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
            var_204 = null;
            _disposed = true;
            var_32 = null;
         }
